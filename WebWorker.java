...
public void run()
{
   ...
   try {
	 ...
	 Error404(os);
      ...
      System.err.println("Output1 error: "+e);
   }
...
private void readHTTPRequest(InputStream is, OutputStream os)
{
   ...
   while (true) {
	 ...
	 if ( line.substring(0,3).compareTo("GET") == 0 ){ 
		...
		// check content of the file type
		...
		contentType = Files.probeContentType(source);
		   ...
		   str = str.replaceAll( "<cs371server>", "Brandons Server" );
	        ...
	        else if ( contentType.compareTo("image/png") == 0 ){
		   ErrorFound = 1;
		   writeHTTPHeader(os,"image/png");
		   byte[] outFile = Files.readAllBytes(htmlfile.toPath());
		   os.write(outFile);
	        } // end else if
		else if ( contentType.compareTo("image/jpeg") == 0 ){
		   ErrorFound = 1;
		   writeHTTPHeader(os,"image/jpeg");
		   byte[] outFile = Files.readAllBytes(htmlfile.toPath());
		   os.write(outFile);
		}
		else if ( contentType.compareTo("image/gif") == 0 ){
		   ErrorFound = 1;
